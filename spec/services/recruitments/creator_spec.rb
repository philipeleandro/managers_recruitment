...
RSpec.describe Recruitments::Creator, type: :service do
  ...
  let(:invalid_params) { { recruitment: recruitment_params.except(:description).merge(recruitment_role_attributes: roles_data) } }
  ...
  describe '#call' do
    ...
    context 'with valid params' do
      ...
      context 'assigns correct roles and quantities' do
        subject(:resource) { service_call[:resource] }
      ...
      context 'returns the created recruitment and no errors' do
    ...
    context 'with invalid params' do
      ...
      context 'returns the unsaved recruitment and errors' do
        subject(:result) { service_call }
    ...
    context 'when a standard error is raised' do
      ...
      before do
        allow_any_instance_of(described_class).to receive(:recruitment_roles_creation).and_raise(StandardError, 'Something went wrong')
      end
